...
const Google = () => {
  ...
  const [state, setState] = useState({ issuer: "Google", notes: "" });
  useEffect(() => {
    const event = async () => {
  ...
  // Category filtering
  const googleCategories = store.categories.filter(
    (category) => category.issuer === "Google"
  );
  const availableCurrency = googleCategories.map((cur) => cur.currency);
  const availableSize = googleCategories
    .filter((category) => category.currency === state.currency)
    .map((cat) => cat.card_type);
  const finalCategory = googleCategories
    .filter((category) => category.currency === state.currency)
