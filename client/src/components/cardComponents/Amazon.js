...
const Amazon = () => {
  ...
  const [state, setState] = useState({ issuer: "Amazon", notes: "" });
  useEffect(() => {
    const event = async () => {
  ...
  const amazonCategories = store.categories.filter(
    (category) => category.issuer === "Amazon"
  );
  const availableCurrency = amazonCategories.map((cur) => cur.currency);
  const availableSize = amazonCategories
    .filter((category) => category.currency === state.currency)
    .map((cat) => cat.card_type);
  const finalCategory = amazonCategories
    .filter((category) => category.currency === state.currency)
