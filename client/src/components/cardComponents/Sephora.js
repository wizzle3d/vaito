...
const Sephora = () => {
  ...
  const [state, setState] = useState({ issuer: "Sephora", notes: "" });
  useEffect(() => {
    const event = async () => {
  ...
  // Category filtering
  const sephoraCategories = store.categories.filter(
    (category) => category.issuer === "Sephora"
  );
  const availableCurrency = sephoraCategories.map((cur) => cur.currency);
  const availableSize = sephoraCategories
    .filter((category) => category.currency === state.currency)
    .map((cat) => cat.card_type);
  const finalCategory = sephoraCategories.filter(
