...
const Ebay = () => {
  ...
  const [state, setState] = useState({ issuer: "ebay", notes: "" });
  useEffect(() => {
    const event = async () => {
  ...
  // Category filtering
  const ebayCategories = store.categories.filter(
    (category) => category.issuer === "ebay"
  );
  const availableCurrency = ebayCategories.map((cur) => cur.currency);
  const availableSize = ebayCategories
    .filter((category) => category.currency === state.currency)
    .map((cat) => cat.card_type);
  const finalCategory = ebayCategories.filter(
