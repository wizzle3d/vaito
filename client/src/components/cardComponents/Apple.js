...
const Apple = () => {
  ...
  const [state, setState] = useState({ issuer: "Apple", notes: "" });
  useEffect(() => {
    const event = async () => {
  ...
  // Category filtering
  const appleCategories = store.categories.filter(
    (category) => category.issuer === "Apple"
  );
  const availableCurrency = appleCategories.map((cur) => cur.currency);
  const availableSize = appleCategories
    .filter((category) => category.currency === state.currency)
    .map((cat) => cat.card_type);
  const finalCategory = appleCategories
    .filter((category) => category.currency === state.currency)
