...
const Itunes = () => {
  ...
  const [state, setState] = useState({ issuer: "iTunes", notes: "" });
  useEffect(() => {
    const event = async () => {
  ...
  // Category filtering
  const itunesCategories = store.categories.filter(
    (category) => category.issuer === "iTunes"
  );
  const availableCurrency = itunesCategories.map((cur) => cur.currency);
  const availableSize = itunesCategories
    .filter((category) => category.currency === state.currency)
    .map((cat) => cat.card_type);
  const finalCategory = itunesCategories.filter(
