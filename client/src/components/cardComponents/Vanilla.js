...
const Vanilla = () => {
  ...
  const [state, setState] = useState({ issuer: "Vanilla", notes: "" });
  useEffect(() => {
    const event = async () => {
  ...
  // Category filtering
  const vanillaCategories = store.categories.filter(
    (category) => category.issuer === "Vanilla"
  );
  const availableCurrency = vanillaCategories.map((cur) => cur.currency);
  const availableSize = vanillaCategories
    .filter((category) => category.currency === state.currency)
    .map((cat) => cat.card_type);
  const finalCategory = vanillaCategories
    .filter((category) => category.currency === state.currency)
    .filter((cat) => cat.card_type === state.card_type)[0];
  // const {
  //   register,
  //   handleSubmit,
  //   formState: { errors },
