...
const Steam = () => {
  ...
  const [state, setState] = useState({ issuer: "Steam", notes: "" });
  useEffect(() => {
    const event = async () => {
  ...
  // Category filtering
  const steamCategories = store.categories.filter(
    (category) => category.issuer === "Steam"
  );
  const availableCurrency = steamCategories.map((cur) => cur.currency);
  const availableSize = steamCategories
    .filter((category) => category.currency === state.currency)
    .map((cat) => cat.card_type);
  const finalCategory = steamCategories
    .filter((category) => category.currency === state.currency)
